...
function Enemy-Attack {
    ...
    If($Enemy.Health -gt 0){
        ...
        If($EnemyChoice -eq $Enemy.ATKSkill){
            Write-Host "The" $Enemy.Name "uses it's $($Enemy.ATKSkill) ability!" -ForegroundColor Yellow
        ...
        If($EnemyChoice -eq $Enemy.MGKSkill){
            Write-Host "The" $Enemy.Name "uses it's" $Enemy.MGKSkill "ability!" -ForegroundColor Yellow
    ...
    # Need to add a check for Item1/2 -eq $true and see if Player wants to swap.
        ...
        If ($Roll -gt 4) {
            ...
            if ($enemy.Name -eq "Spider") {
                    ...
                    # Changing Prebuff Attack incase of Potion use or something else.
